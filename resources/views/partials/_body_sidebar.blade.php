@php
    ...
    $MyNavBar = \Menu::make('MenuList', function ($menu) use($url){
        ...
        $menu->add('<span>'.__('message.rewards').'</span>', ['class' => ''])
            ...
            ->nickname('rewards')
            ->data('permission', 'rewards list')
            ->link->attr(['class' => ''])
            ->href('#rewards');

            $menu->rewards->add('<span>'.__('message.show_all').'</span>', ['class' => 'sidebar-layout' ,'route' => ['rewards.index', 'show_all' => true]])
                ->data('permission', 'rewards riders list')
                ->prepend('<i class="fas fa-list"></i>')
                ->link->attr(['class' => '']);

            $menu->rewards->add('<span>'.__('message.riders_reward').'</span>', ['class' => 'sidebar-layout' ,'route' => ['rewards.index', 'user_type' => 'rider'] ])
                ->data('permission', 'rewards riders list')
                ->prepend('<i class="fas fa-list"></i>')
                ->link->attr(['class' => '']);

           $menu->rewards->add('<span>'.__('message.drivers_reward').'</span>', ['class' => 'sidebar-layout' ,'route' => ['rewards.index', 'user_type' => 'driver'] ])
                ->data('permission', 'rewards drivers list')
                ->prepend('<i class="fas fa-list"></i>')
                ->link->attr(['class' => '']);
