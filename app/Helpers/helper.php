        ...
        foreach($sendTo as $send){
            ...
            if($user != null )
            {
                ...
                if ( $send != 'admin' && $data['history_type'] != 'new_ride_requested') {
                    ...
                    $notification_data['success_type'] = $data['history_type'];
                    $notification_data['success_message'] = $data['history_message'];
                    $notification_data['result'] = new RideRequestResource($ride_request);
                    $notification_data['clickable'] = "0";
                    $user->notify(
                        new FirebaseNotify(
                            [
                                'title' => $data['history_type'],
                                'body' => $data['history_message'],
            ...
            if( $user == null && isset($ride_request->riderequest_in_driver_id) && $ride_request->riderequest_in_driver != null && $data['history_type'] == 'canceled' ) {
                ...
                $notification_data['result'] = new RideRequestResource($ride_request);
                $notification_data['clickable'] = "0";
                $_user = User::find($ride_request->riderequest_in_driver_id);
                $_user->notify(
                    new FirebaseNotify(
                        [
                            'title' => $data['history_type'],
