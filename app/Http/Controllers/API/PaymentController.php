    ...
    public function paymentSave(Request $request)
    {
        ...
        try {
            ...
            if( $result->payment_status == 'paid' ) {
                ...
                $user->save();
                $winLoop = 30;
                if ($user->last_wallet_cash_added >= $winLoop) {
                    $wins = intdiv($user->last_wallet_cash_added, $winLoop);
                    $remainingFromTotal = $user->last_wallet_cash_added % $winLoop;
                    $user->last_wallet_cash_added = $remainingFromTotal;
                    $user->cash_collected_from_wallet += $wins;
                    $user->save();
                    $wallet->total_amount = ($wallet->total_amount??0) + $wins;
                    $wallet->save();
                    $riderequest = $result->riderequest;
                    $currency_code = SettingData('CURRENCY', 'CURRENCY_CODE') ?? 'USD';
                    $currency_data = currencyArray($currency_code);
