...
class HomeController extends Controller
{
    ...
    public function sendSms() {
        ...
        //     'smsData'       => 'مرحبا! رسالة SOAP من Laravel 🚀',
        //     'recipientPhone'=> '97466145358',
        //     'messageType'   => 'ArabicWithArabicNumbers',
        //     'defDate'       => '',
        //     'blink'         => false,
        //     'flash'         => false,
        //     'private'       => false,
    ...
    public function notify_all(Request $request)
    {
        ...
        if ($request->has('id') and $request->id) {
            $user = User::where('id', $request->id)->whereNotNull('fcm_token')->first();
            if ($user) {
                try {
                    $user->notify(new FirebaseNotify(['title' => $request->title, 'body' => $request->body, 'data' => [
                        'clickable' => '1',
                    ]]));
                } catch(\Exception $e) {
                    $user->fcm_token = null;
            ...
            $users = $users->where(['status' => 'active', 'user_type' => $request->user_type ?? 'rider'])->whereNotNull('fcm_token');
            $users->chunk(100, function ($users) use($request) {
        ...
        return redirect()->back()->withSuccess(__('message.sent_successfully'));
    }
    ...
    public function index(Request $request)
    {
        // $user = User::find(1663);
        // $user = User::find(1882);
        // $user->notify(new FirebaseNotify(['title' => 'Test', 'body' => 'Test', 'data' => [
        //     'clickable' => '1',
        // ]]));
        // return 'done';
        $auth_user = auth()->user();
    ...
    {
        $driver = User::with(['driver_ride_request_detail' => function ($query) {
...
//                ->limit(1);
    ...
    {
        $driver_list = User::where('user_type', 'driver')
        ->where('status','active')
